...
public interface Connection {
    ...
    /**
     * Set the proxy to use for this request. Set to <code>null</code> to
     * disable.
     *
     ...
     * IOException will be thrown. The default timeout is <b>30 seconds</b>
     * (30,000 millis). A timeout of zero is treated as an infinite timeout.
     * <p>
     * Note that a read timeout is not the same as a maximum timeout. As long as
     * the connection is sending bytes at least every <i>timeout</i> seconds
     * (e.g. in the case of an infinite stream of data, or a slow large
     * download), the read timeout will not fire. This can be mitigated by using
     * a maximum download size (see {@link #maxBodySize(int)}), or interrupting
     * the connecting thread after a max timeout.</p>
     *
     ...
     *
     * @param ignoreHttpErrors - false (default) if HTTP errors should be
     * ignored.
     * @return this Connection, for chaining
     */
     ...
     *
     * @param ignoreContentType set to true if you would like the content type
     * ignored on parsing the response into a Document.
     * @return this Connection, for chaining
     */
     ...
     * </p>
     * <p>
     * Some servers use expired, self-generated certificates; or your JDK may
     * not support SNI hosts. In which case, you may want to enable this
     * setting.
     * </p>
     * <p>
     ...
     * </p>
     *
     * @param value if should validate TLS (SSL) certificates. <b>true</b> by
     * default.
     * @return this Connection, for chaining
     */
     ...
     * but for POSTS this will upload the input stream.
     *
     * @param key data key (form item name)
     * @param filename the name of the file to present to the remove server.
     * Typically just the name, not path, component.
     * @param inputStream the input stream to upload, that you probably obtained
     * from a {@link java.io.FileInputStream}. You must close the InputStream in
     ...
     * request body, not a set for URL encoded form key/value pairs. E.g.:      <code><pre>Jsoup.connect(url)
     * .requestBody(json)
     ...
     * Document. If not set, defaults to the HTML parser, unless the response
     * content-type is XML, in which case the XML parser is used.
     *
    ...
    interface Request extends Base<Request> {
        ...
        /**
         * Set a POST (or PUT) request body. Useful when a server expects a
         * plain request body, not a set for URL encoded form key/value pairs.
         * E.g.:          <code><pre>Jsoup.connect(url)
         * .requestBody(json)
         * .header("Content-Type", "application/json")
         * .post();</pre></code> If any data key/vals are supplied, they will be
         * sent as URL query params.
