...
public abstract class Node implements Cloneable {
     ...
     * null) if the attribute was missing or could not be made successfully into
     * a URL.
     * @see #attr
    ...
    /**
     * Gets this node's parent node. Not overridable by extending classes, so
     * useful if you really just need the Node type.
     *
     * @return parent node; or null if no parent.
     */
    ...
    /**
     * Get this node's root node; that is, its topmost ancestor. If this node is
     * the top ancestor, returns {@code this}.
     *
     ...
     * </p>
     * <p>
     * {@code <div>One <span>Two <b>Three</b></span></div>}</p>
     * Calling {@code element.unwrap()} on the {@code span} element will result
     * in the html:
     * <p>
     * {@code <div>One Two <b>Three</b></div>}</p>
     ...
     * {@link #childNodes()  node.parent.childNodes()}, but does not include
     * this node (a node is not a sibling of itself).
     *
    ...
    /**
     * Check if this node is the same instance of another (object identity
     * test).
     *
     ...
     * considered the same if its name, attributes and content match the other
     * node; particularly its position in the tree does not influence its
     * similarity.
     *
     * @param o other object to compare to
     * @return true if the content of this node is the same as the other
