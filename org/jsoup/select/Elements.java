...
public class Elements extends ArrayList<Element> {
     ...
     * attribute.. If no elements were matched (isEmpty() == true), or if the no
     * elements have the attribute, returns empty string.
     * @see #hasAttr(String)
     */
     ...
     * does not have this attribute, no value is included in the result set for
     * that element.
     *
     * @param attributeKey the attribute name to return values for. You can add
     * the {@code abs:} prefix to the key to get absolute URLs from relative
     * URLs, e.g.: {@code doc.select("a").eachAttr("abs:href")} .
     ...
     *
     * @param className class name to add if missing, or remove if present, from
     * every element.
     * @return this
     */
    ...
    /**
     * Get the text content of each of the matched elements. If an element has
     * no text, then it is not included in the result.
     *
     ...
     *
     * @param html HTML to wrap around each element, e.g.
     ...
     * E.g. with HTML:
     * <p>
     * {@code <div><font>One</font> <font><a href="/">Two</a></font></div>}</p>
     * <p>
     * {@code doc.select("font").unwrap();}</p>
     * <p>
     * HTML = {@code <div>One <a href="/">Two</a></div>}</p>
     *
     * @return this (for chaining)
     * @see Node#unwrap
     */
     ...
     * <p>
     * @param query the selector query whose results should be removed from
     * these elements
